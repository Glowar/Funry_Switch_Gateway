...
def configMsg (key):
    return '{"name":"Key ' + ('ALL' if key == 255 else '{:0>3}'.format(str(key))) + '","command_topic":"'+ config.MQTT_TOPIC + '/Set","payload_on":"{ \\"Key\\": \\"' + str(key) + '\\", \\"State\\": \\"1\\" }","payload_off":"{ \\"Key\\": \\"' + str(key) + '\\", \\"State\\": \\"0\\" }","state_topic":"' + config.MQTT_TOPIC + '/Key' + str(key) + '","state_on":"1","state_off":"0","optimistic":"true","retain":"false","unique_id":"Key' + str(key) + '","device":{"identifiers":["Funry"],"name":"Funry"}}'
    ...
    while True:
        if mqttcon == 1:
            ...
            if config.qSwitch2Mqtt:
                ...
                mqttc.publish(config.MQTT_TOPIC + '/Key' + str(k.key), k.state)
